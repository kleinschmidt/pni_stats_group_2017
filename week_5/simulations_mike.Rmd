...
gen_synth_dataset <- function(n_subjects, n_items, n_repetitions, intecept, effect_size, subject_ranef_cov, item_ranef_cov){
...
Now, to build intuition, we can simulate a lot of synthetic datasets and analyze them. To do this, we write a function that generates synthetic data, analyzes it, and extracts the stats. Then we run it many times and look at the distribution of outcomes. First, our function:
...
nSims <- 1000 # 100 is fast but can be unstable; 1000 is good
...
```{r}
run_repeated_anova <- function(param_list){
  d <- with(param_list, gen_synth_dataset(n_subjects, n_items, n_repetitions, intercept, effect_size, subject_ranef_cov, item_ranef_cov))
...
```{r}
param_list_null <- list(n_subjects = 10, n_items = 5, n_repetitions = 20, intercept=5, effect_size = 0 ,subject_ranef_cov=diag(2), item_ranef_cov=diag(2))
fx_null_repeated_measures <- ldply(1:nSims, function(x) run_repeated_anova(param_list_null), .parallel=TRUE) %>% as.data.table
...
```{r}
run_ttest_on_betas <- function(param_list){
  d <- with(param_list, gen_synth_dataset(n_subjects, n_items, n_repetitions, intercept, effect_size, subject_ranef_cov, item_ranef_cov))
...
```{r}
param_list_null <- list(n_subjects = 10, n_items = 5, n_repetitions = 20, intercept=5, effect_size = 0 ,subject_ranef_cov=diag(2), item_ranef_cov=diag(2))
fx_null_ttest_betas <- ldply(1:nSims, function(x) run_ttest_on_betas(param_list_null), .parallel=TRUE) %>% as.data.table
...
library(lme4)
param_list_null <- list(n_subjects = 10, n_items = 5, n_repetitions = 20, intercept=5, effect_size = 0 ,subject_ranef_cov=diag(2), item_ranef_cov=diag(2))
fx_null_lmer <- ldply(1:nSims, function(x) run_lmer(param_list_null), .parallel=TRUE) %>% as.data.table
...
```{r}
param_list <- list(n_subjects = 10, n_items = 5, n_repetitions = 20, intercept=5, effect_size = 0.2 ,subject_ranef_cov=diag(2), item_ranef_cov=diag(2))
fx_lmer <- ldply(1:nSims, function(x) run_lmer(param_list), .parallel=TRUE) %>% as.data.table
